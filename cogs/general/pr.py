...
async def create_embeds(res):
    ...
    return power_1v1, power_2v2, stats

class PowerSelect(discord.ui.Select):
    def __init__(self, res, view, msg):
        super().__init__(placeholder="Select a player", options=[discord.SelectOption(label=f"{r['player']['name']}", value=f"{i}") for i, r in enumerate(res)])
        self.res = res
        self.power_view = view
        self.msg = msg
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if retry:
            embed = discord.Embed(description="**Command on cooldown, try again in {:.1f}s**".format(retry), colour=error_colour)
            await interaction.response.send_message(embed=embed, ephemeral=True)
        await interaction.response.defer()
        power_1v1, power_2v2, stats = await create_embeds(self.res[int(self.values[0])])
        self.power_view.embeds = [power_1v1, power_2v2]
        self.power_view.pages = stats
        await self.msg.edit(embed=power_1v1, view=self.power_view)

class PowerButtons(discord.ui.View):
    def __init__(self, ctx, embeds, response, pages, res):
        super().__init__(timeout=300)
        self.add_item(PowerSelect(res=res, view=self, msg=response))
...
class Pr(commands.Cog, name="PR"):
    ...
    async def pr(self, ctx, *, name=None):
        ...
        try: 
            all_res = req.json()["searchPlayers"]
